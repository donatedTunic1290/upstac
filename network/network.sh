...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile CommonChannel -outputAnchorPeersUpdate ./channel-artifacts/hospitalAMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg hospitalAMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile CommonChannel -outputAnchorPeersUpdate ./channel-artifacts/hospitalBMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg hospitalBMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile CommonChannel -outputAnchorPeersUpdate ./channel-artifacts/governmentMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg governmentMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
