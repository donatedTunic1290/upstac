...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating HospitalA Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-hospitalA.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating HospitalB Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-hospitalB.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/hospitalA/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for HospitalA, HospitalB and Government"
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile UpstacOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
# call the script to create the channel, join the peers of hospitalA and hospitalB,
...
function createChannel() {
  ...
  scripts/createChannelHospitalB.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
}


## Call the script to deploy a chaincode to the channel
function deployCC() {
  scripts/deployCCHospitalB.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  # stop org3 containers also in addition to hospitalA and hospitalB, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/hospitalA/msp organizations/fabric-ca/hospitalA/tls-cert.pem organizations/fabric-ca/hospitalA/ca-cert.pem organizations/fabric-ca/hospitalA/IssuerPublicKey organizations/fabric-ca/hospitalA/IssuerRevocationPublicKey organizations/fabric-ca/hospitalA/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/hospitalB/msp organizations/fabric-ca/hospitalB/tls-cert.pem organizations/fabric-ca/hospitalB/ca-cert.pem organizations/fabric-ca/hospitalB/IssuerPublicKey organizations/fabric-ca/hospitalB/IssuerRevocationPublicKey organizations/fabric-ca/hospitalB/fabric-ca-server.db'
...
COMPOSE_FILE_BASE=docker/docker-compose-upstac.yaml
