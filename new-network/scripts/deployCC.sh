...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.upstac.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.upstac.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.upstac.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
## Install chaincode on peer0.hospitalA and peer0.hospitalB
infoln "Installing chaincode on peer0.hospitalA..."
installChaincode "hospitalA"
infoln "Install chaincode on peer0.hospitalB..."
installChaincode "hospitalB"
infoln "Install chaincode on peer0.government..."
installChaincode "government"

## query whether the chaincode is installed
queryInstalled "hospitalA"

## approve the definition for hospitalA
approveForMyOrg "hospitalA"

## approve the definition for government
approveForMyOrg "government"

## check whether the chaincode definition is ready to be committed
## expect hospitalA to have approved and hospitalB not to
checkCommitReadiness "hospitalA" "\"HospitalAMSP\": true" "\"HospitalBMSP\": false" "\"GovernmentMSP\": true"
checkCommitReadiness "hospitalB" "\"HospitalAMSP\": true" "\"HospitalBMSP\": false" "\"GovernmentMSP\": true"

## now approve also for hospitalB
approveForMyOrg "hospitalB"

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness "hospitalA" "\"HospitalAMSP\": true" "\"HospitalBMSP\": true" "\"GovernmentMSP\": true"
checkCommitReadiness "hospitalB" "\"HospitalAMSP\": true" "\"HospitalBMSP\": true" "\"GovernmentMSP\": true"
checkCommitReadiness "government" "\"HospitalAMSP\": true" "\"HospitalBMSP\": true" "\"GovernmentMSP\": true"

## now that we know for sure both orgs have approved, commit the definition
commitChaincodeDefinition "hospitalA" "hospitalB" "government"

## query on both orgs to see that the definition committed successfully
queryCommitted "hospitalA"
queryCommitted "hospitalB"
