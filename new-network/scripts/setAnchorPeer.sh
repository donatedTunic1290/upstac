...
updateAnchorPeer() {
  peer channel update -o orderer.upstac.com:7050 --ordererTLSHostnameOverride orderer.upstac.com -c $CHANNEL_NAME -f ${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile $ORDERER_CA >&log.txt
  res=$?
  cat log.txt
